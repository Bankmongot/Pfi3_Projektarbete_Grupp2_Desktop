...
public class BottleTheme extends JPanel implements ThemeInterface {
	...
	ArrayList<Image> labels = new ArrayList<Image>();
	Image bottle1;
	Image bottle2;
	Image bcgr;
	Image table;
	...
	public BottleTheme(){
		...
		labelGradient = Toolkit.getDefaultToolkit().getImage(VerticalBoxes.class.getResource("/images/bottleLabelGradient.png"));
		...
		labels.add(Toolkit.getDefaultToolkit().getImage(VerticalBoxes.class.getResource("/images/labelone.png")));
		labels.add(Toolkit.getDefaultToolkit().getImage(VerticalBoxes.class.getResource("/images/labeltwo.png")));
		labels.add(Toolkit.getDefaultToolkit().getImage(VerticalBoxes.class.getResource("/images/labelthree.png")));
		labels.add(Toolkit.getDefaultToolkit().getImage(VerticalBoxes.class.getResource("/images/labelfour.png")));
		labels.add(Toolkit.getDefaultToolkit().getImage(VerticalBoxes.class.getResource("/images/labelfive.png")));
		labels.add(Toolkit.getDefaultToolkit().getImage(VerticalBoxes.class.getResource("/images/labelsix.png")));
	...
	public void paintComponent(Graphics g) {
		...
		FontMetrics metrics = g2.getFontMetrics(fontTitle);
		int titleWidth = metrics.stringWidth(question);
		int titleXpos = (int)((windowWidth - titleWidth)/2.0);

		g2.drawString(question, titleXpos, tableY-(fontSize*2)); //Answer, aligned by the biggest box
		...
		for (Bottle bottle : bottles){
			...
			int labelWidth = (int)(bottleWidth-(bottleWidth/3.0));
			int bottleX = ((bottleCount*spaceBetween)+xStart);
			int bottleY = (int)(tableY+(50*scale));
			//Draw bottle
			g2.drawImage(bottle.image, bottleX, bottleY, bottleWidth, bottleHeight, this); //Draw bottle, aligned with the table.
			g2.drawImage(bottle.label, (int)(bottleX+((bottleWidth-labelWidth)/2.0)), (int)(bottleY+(bottleHeight/2.0)), labelWidth, (int)(labelWidth-(labelWidth/2.0)), this); //Draw bottle, aligned with the table.
			g2A.drawImage(labelGradient, (int)(bottleX+((bottleWidth-labelWidth)/2.0)),  (int)(bottleY+(bottleHeight/2.0)), labelWidth, (int)(labelWidth-(labelWidth/2.0)), this);
			//Draw text
			g2.drawString((bottleCount + 1) + ". " + bottle.answer + " (" + bottle.votes + ")", (int) graphWidth, (int)((fontSize*2.0)*bottleCount)+tableY+fontSize); //Answer, aligned by the biggest box
			bottleCount++;
		}
		g2.dispose();
	}
	...
	public void updateData(FirebaseData data) {
		...
		for(int i = bottles.size(); i<answers.size(); i++){
			if(Math.random() > 0.5) bottles.add(new Bottle(bottle1, colors.get(i), labels.get(i)));
			else bottles.add(new Bottle(bottle2, colors.get(i), labels.get(i)));
	...
	class Bottle{
		...
		Bottle(Image img, Color c, Image l){ this.image = img; this.color = c; this.label = l;}
